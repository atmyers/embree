...
public:
	class iterator : std::iterator<std::random_access_iterator_tag, Thread*>{
		friend class ThreadPool;
		
	private:
		Worker** p;
		
		iterator(Worker** p) : p(p) {
		}
	...
	ThreadPool(unsigned threadCount = cpuCount()) {
		exit = false;
		currentWorkItem = 0;
		addedWorkItems = 0;
		
		taskQueue = static_cast<TaskInfo*>(_mm_malloc(sizeof(TaskInfo)*maxTaskCount, 64));
		taskCount = 0;

		threads.resize(threadCount, 0);
	...
	iterator begin() {
		return iterator(&threads[0]);
	}
	
	iterator end() {
		return iterator(&threads[0] + threads.size());
	}
